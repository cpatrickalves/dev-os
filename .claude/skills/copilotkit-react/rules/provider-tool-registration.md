...
**Alternative (provider-level tool renderers with renderToolCalls prop):**
...
Use `renderToolCalls` on the provider when tool renderers should be available globally and don't depend on component-level state. Prefer hook-based registration when renderers need access to local state or should follow component lifecycle.

Reference: [useCopilotAction](https://docs.copilotkit.ai/reference/v1/hooks/useCopilotAction) | [useFrontendTool (v2)](https://docs.copilotkit.ai/reference/v2/hooks/useFrontendTool) | [CopilotKit Provider](https://docs.copilotkit.ai/reference/v2/components/CopilotKit)
