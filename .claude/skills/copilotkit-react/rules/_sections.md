...
**Impact:** HIGH
**Description:** Patterns for useAgent (v2), useFrontendTool (v2), useHumanInTheLoop, useCopilotReadable (v1), and useCopilotAction (v1). Incorrect usage causes re-render storms, stale state, or broken agent interactions.

## 3. Tool Rendering (tool)

**Impact:** HIGH
**Description:** Rules for rendering agent tool calls in the UI using useRenderTool (v2), useRenderToolCall, and useDefaultTool. Proper tool rendering is what makes CopilotKit's generative UI possible.
