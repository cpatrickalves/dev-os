...
Architecture and implementation patterns for building AI agents that integrate with CopilotKit via the AG-UI protocol. Contains 23 rules covering agent design with BuiltInAgent (@copilotkitnext/agent), useAgent hook, hook migration (useFrontendTool, useHumanInTheLoop, useRenderToolCall), multi-framework AG-UI adapters (Pydantic AI, LlamaIndex, MS Agent Framework), event streaming, state synchronization, human-in-the-loop checkpoints, and generative UI emission.
...
1. [Agent Architecture](#1-agent-architecture) — **CRITICAL**
   - 1.1 [Use BuiltInAgent for Direct-to-LLM Agents](#11-use-builtinagent-for-direct-to-llm-agents)
   - 1.2 [Use useAgent Hook for React Agent Integration](#12-use-useagent-hook-for-react-agent-integration)
   - 1.3 [Migrate from useCopilotAction to Specialized Hooks](#13-migrate-from-usecopilotaction-to-specialized-hooks)
   - 1.4 [Use Native AG-UI Integration for Your Agent Framework](#14-use-native-ag-ui-integration-for-your-agent-framework)
   - 1.5 [Use Provider/Model String Format for Model Selection](#15-use-providermodel-string-format-for-model-selection)
   - 1.6 [Set maxSteps to Prevent Infinite Loops](#16-set-maxsteps-to-prevent-infinite-loops)
   - 1.7 [Configure MCP Servers for External Tools](#17-configure-mcp-servers-for-external-tools)
2. [AG-UI Protocol](#2-ag-ui-protocol) — **HIGH**
   - 2.1 [Emit AG-UI Events in Correct Order](#21-emit-ag-ui-events-in-correct-order)
   - 2.2 [Always Emit Error Events on Failure](#22-always-emit-error-events-on-failure)
...
4. [Human-in-the-Loop](#4-human-in-the-loop) — **MEDIUM**
   - 4.1 [Use Tool Calls for Approval Gates](#41-use-tool-calls-for-approval-gates)
   - 4.2 [Include Context for User Decisions](#42-include-context-for-user-decisions)
   - 4.3 [Preserve State When Resuming After Approval](#43-preserve-state-when-resuming-after-approval)
   - 4.4 [Set Timeouts with Fallback Behavior](#44-set-timeouts-with-fallback-behavior)
5. [Generative UI Emission](#5-generative-ui-emission) — **MEDIUM**
   - 5.1 [Emit Tool Calls That Map to Frontend Render Hooks](#51-emit-tool-calls-that-map-to-frontend-render-hooks)
...
### 1.1 Use BuiltInAgent for Direct-to-LLM Agents
...
### 1.2 Use useAgent Hook for React Agent Integration

**Impact: CRITICAL (useAgent is the primary interface for agent state, subscriptions, and multi-agent execution)**
...
### 1.3 Migrate from useCopilotAction to Specialized Hooks

**Impact: HIGH (useCopilotAction is deprecated; using it causes type safety issues and unclear intent)**
...
### 1.4 Use Native AG-UI Integration for Your Agent Framework

**Impact: HIGH (native AG-UI adapters eliminate manual event handling and reduce boilerplate)**

AG-UI is now supported natively by multiple agent frameworks. Use the built-in AG-UI adapter for your framework instead of implementing manual event streaming.
...
### 1.5 Use Provider/Model String Format for Model Selection
...
### 1.6 Set maxSteps to Prevent Infinite Loops
...
### 1.7 Configure MCP Servers for External Tools
...
### 2.1 Emit AG-UI Events in Correct Order
...
### 2.2 Always Emit Error Events on Failure
...
Tool calls require a complete event lifecycle: `TOOL_CALL_START` -> `TOOL_CALL_ARGS` (streamed) -> `TOOL_CALL_END`. Missing any step causes the frontend's `useRenderToolCall` to hang in an incorrect status or miss the tool call entirely.
...
### 4.1 Use Tool Calls for Approval Gates
...
### 4.2 Include Context for User Decisions
...
### 4.3 Preserve State When Resuming After Approval
...
### 4.4 Set Timeouts with Fallback Behavior
...
### 5.1 Emit Tool Calls That Map to Frontend Render Hooks
...
- https://docs.copilotkit.ai/direct-to-llm/guides/generative-ui
- https://docs.copilotkit.ai/direct-to-llm/guides/use-agent-hook
